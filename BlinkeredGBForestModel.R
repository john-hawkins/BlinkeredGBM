...
##########################################################################
BlinkeredGBForestModel = function(x, y, tset, vset, learning_rate=0.2, blinker=NULL, delta=NULL, tolerance=0.001, epochs=10) {
	...
	base.model		<- randomForest(mod.formula, data=trainset, ntree=20)
	...
	while( improvement >= tolerance && cur.blinker > 0 && nmodz < epochs) {
		...
		splitterz[[nmodz]]        <- randomForest(seg.formula, data=seg.training, ntree=20)
		...
		regressrz[[nmodz]]  	<- randomForest(reg.formula, data=temp.within.train, ntree=20)
        ...
        new("BlinkeredGBForestModel", features=x, target=y, 
...
setMethod(
        f="forecast",
        signature="BlinkeredGBForestModel",
