#################################################################################
# Build a gradient boosted model using the GLM function as the
...
##########################################################################
BlinkeredGBLinearModel = function(x, y, tset, vset, learning_rate=0.2, blinker=NULL, delta=NULL, tolerance=0.001, epochs=10) {
	...
	base.model		<- glm(mod.formula, data=trainset)
	...
	while( improvement >= tolerance && cur.blinker > 0 && nmodz < epochs) {
		...
		# FORCE THE ORDER OF LEVELS TO BE SUCH THAT 'OUTSIDE' WILL BE ZERO ENCODED
		# trn.seg <- cbind(factor(trn.seg, levels=c("outside","between")))
		seg.training$error_segment <- factor(seg.training$error_segment, levels=c("outside","between"))
                ...
                # FORCE THE ORDER OF LEVELS TO BE SUCH THAT 'OUTSIDE' WILL BE ZERO ENCODED
                # vd.seg <- factor(vd.seg, levels=c("outside","between"))
                seg.validation$error_segment <- factor(seg.validation$error_segment, levels=c("outside","between"))
		...
		splitterz[[nmodz]]      <- glm(seg.formula, data=seg.training, family=binomial(link='logit'))
		...
		regressrz[[nmodz]]  	<- glm(reg.formula, data=temp.within.train)
			
		### NOW WE GENERATE THE FORECASTS AND ERROR FOR THE NEW MODEL AND ADD THEM AS COLUMNS
		trn.seg.preds		<- cbind(predict( object=splitterz[[nmodz]], newdata=training, type='response') )
		vd.seg.preds		<- cbind(predict( object=splitterz[[nmodz]], newdata=validation, type='response') )
		...
		trn.fcast		<- training[[forecast.col]] - learning_rate * gammaz[[nmodz]] * ( trn.seg.preds * trn.reg.preds)
		vd.fcast        	<- validation[[forecast.col]] - learning_rate * gammaz[[nmodz]] * (vd.seg.preds * vd.reg.preds)
        ...
        new("BlinkeredGBLinearModel", features=x, target=y, 
...
setMethod(
        f="forecast",
        signature="BlinkeredGBLinearModel",
        definition=function(theObject, dataset, useClassifiers=TRUE)
	{
		...
		currpred			= basepreds
		   ...
		   totalFcasts[[mdel]]		<- currpred - ( 
				#theObject@learning_rate * theObject@gammas[[mdel]] * ( ifelse(useClassifiers,splitterFcasts[[mdel]],1) * regressFcasts[[mdel]] )
				theObject@learning_rate * theObject@gammas[[mdel]] * ( splitterFcasts[[mdel]] * regressFcasts[[mdel]] )
			)
